...
const char DEFAULT_WALLET_FILE_NAME[] = "goodnesswallet.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "dyngepeng.zapto.org:3181";
  m_defaultPoolList << "xvv.communitypool.org:3198";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("goodnesswallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("goodnesswallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("goodnesswallet.desktop");
  ...
  res = autorunSettings.contains("BytecoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("GoodnessWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.goodness.goodnesswallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=Goodness Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("GoodnessWallet", appPath);
    } else {
      autorunSettings.remove("GoodnessWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\goodness", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:goodness");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\goodness\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\goodness\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("goodnesswallet.cfg"));
