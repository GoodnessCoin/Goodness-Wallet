...
const char BYTECOIN_URI_SCHEME_NAME[] = "goodness";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("goodnesswallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("goodnesswallet.lock"));
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] Goodness wallet already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "Goodness wallet already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have goodnessd with non-default RPC port, you should terminate it and relaunch Goodness Wallet\n"
      "or\n"
      "Set the Local deamon required port in Goodness Wallet Menu/Preferences/Connection settings."));
      dlg.exec();
      return false;
    }
