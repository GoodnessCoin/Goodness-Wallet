...
const char COMMUNITY_FORUM_URL[] = "https://discord.gg/62cBt9P";
const char REPORT_ISSUE_URL[] = "https://github.com/GoodnessCoin";
...
MainWindow::MainWindow(ICryptoNoteAdapter* _cryptoNoteAdapter, IAddressBookManager* _addressBookManager,
  ...
  m_ui->setupUi(this);
  setWindowTitle(tr("Goodness Wallet %1").arg(Settings::instance().getVersion()));
...
void MainWindow::qrClicked(){
    QRCodeDialog* m_qrcode = new QRCodeDialog(QString("Your Goodness address"),m_ui->m_addressLabel->text().toLocal8Bit(),this);
    m_qrcode->show();
}
